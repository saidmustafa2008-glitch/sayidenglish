...
function activeDateSet(state){return new Set([...state.sessions.filter(s=>s.minutes>0).map(s=>s.date),...state.listening.filter(s=>s.minutes>0).map(s=>s.date),...state.journal.filter(j=>j.text.trim()).map(j=>j.date)])}
export function streak(state){const active=activeDateSet(state);let count=0;for(let i=0;i<500;i++){const k=dayKey(dateShift(-i));if(active.has(k))count++;else if(i===0)continue;else break}return count}
export function longestStreak(state){const days=[...activeDateSet(state)].sort();if(!days.length)return 0;let best=1,cur=1;for(let i=1;i<days.length;i++){const a=new Date(days[i-1]+'T12:00:00'),b=new Date(days[i]+'T12:00:00'),diff=Math.round((b-a)/86400000);if(diff===1){cur++;best=Math.max(best,cur)}else if(diff>1)cur=1}return best}
export function xp(state){const quizCorrect=sum(state.quizHistory,q=>q.score);return Math.round(totalMinutes(state)+totalListening(state)+state.words.length*5+masteredWords(state)*10+finishedBooks(state)*120+state.sessions.filter(s=>s.summary.trim()).length*10+state.journal.length*15+quizCorrect*3+shadowingMinutes(state)*.5)}
export function levelInfo(state){const x=xp(state),level=Math.max(1,Math.floor(x/400)+1),into=x%400;return{xp:x,level,into,next:400,pct:Math.round(into/400*100)}}
export function roadmap(state){const metrics=[Math.min(1,totalMinutes(state)/1200),Math.min(1,totalListening(state)/500),Math.min(1,state.words.length/220),Math.min(1,masteredWords(state)/120),Math.min(1,state.sessions.filter(s=>s.summary.trim()).length/25),Math.min(1,state.journal.length/20),Math.min(1,finishedBooks(state)/3)];return Math.round(sum(metrics)/metrics.length*100)}
...
export const monthItems=(state,date)=>{const y=date.getFullYear(),m=date.getMonth();return{reading:state.sessions.filter(s=>{const d=new Date(s.date+'T12:00:00');return d.getFullYear()===y&&d.getMonth()===m}),listening:state.listening.filter(s=>{const d=new Date(s.date+'T12:00:00');return d.getFullYear()===y&&d.getMonth()===m}),journal:state.journal.filter(j=>{const d=new Date(j.date+'T12:00:00');return d.getFullYear()===y&&d.getMonth()===m})}}
